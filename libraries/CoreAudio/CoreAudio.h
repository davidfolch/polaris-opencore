...
class CoreAudio: public Machine {
 ...
 private:
  ...
  static constexpr float SWING_SENSITIVITY = 120;       // how hard should be a swing to get the max effect. It's in deg/s (good values are between 360 and 720)
  static constexpr float ROLL_SENSITIVITY = 180;        // how hard should be a roll swing to get the max effect. It's in deg/s
  static constexpr float MAXIMUM_HUM_DUCKING = 0.6;    // how much the main hum is reduced during the swing. Close to 1 means that the main hum is attuanuated a lot during the swing (good values between 0.7 and 0.9)
